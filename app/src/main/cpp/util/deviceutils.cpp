...
(new_nougat_open_memory)(void *DexFile_thiz, uint8_t *base, size_t size, void *location,
    ...
    if (size > DEX_MIN_LEN) {
        save_dex_file(base, size);
        __android_log_print(ANDROID_LOG_INFO, TAG, "save_dex_file");
    } else {
        __android_log_print(ANDROID_LOG_INFO, TAG, "size:%zx", size);
    }
...
void *get_new_open_function_addr() {
    if (is_arm_64()) {
        if (is_oreo()) {
            __android_log_print(ANDROID_LOG_INFO, TAG, "get_new_open_function_addr,is_arm_64&is_oreo");
