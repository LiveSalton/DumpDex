...
static void get_file_name(char *name, int len, int dexlen) {
    __android_log_print(ANDROID_LOG_INFO, TAG, "name:%s,/data/data/%s/dump/source-%u.dex", name, pname, dexlen);
    memset(name, 0, len);
}

//保存dex文件
void save_dex_file(u_int8_t *data, size_t length) {
    __android_log_print(ANDROID_LOG_INFO, TAG, "save_dex_file,size:%zx", length);
    char dname[1024];
